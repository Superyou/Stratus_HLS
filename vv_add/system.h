...
, mem_resp_replay_chan("mem_resp_replay_chan") //unknow
...

    {
        ...
        m_vv_add->rst.bind(rst_sig);

        m_vv_add->cc_exception_i(cc_exception_chan);
        m_vv_add->cc_busy_o(cc_busy_chan);
        m_vv_add->cc_interrupt_o(cc_interrupt_chan);
m_vv_add-> cc_status_i(cc_status_chan);
m_vv_add-> cc_host_id_i(cc_host_id_chan);

m_vv_add-> core_cmd_ready_o(core_cmd_ready_chan);
m_vv_add-> core_cmd_valid_i(core_cmd_valid_chan);
m_vv_add-> core_cmd_inst_funct_i(core_cmd_inst_funct_chan);
m_vv_add-> core_cmd_inst_rs1_i(core_cmd_inst_rs1_chan);
m_vv_add-> core_cmd_inst_rs2_i(core_cmd_inst_rs2_chan);

m_vv_add-> core_cmd_inst_xd_i(core_cmd_inst_xd_chan);
m_vv_add-> core_cmd_inst_xs1_i(core_cmd_inst_xs1_chan);
m_vv_add-> core_cmd_inst_xs2_i(core_cmd_inst_xs2_chan);
m_vv_add-> core_cmd_inst_rd_i(core_cmd_inst_rd_chan);
m_vv_add-> core_cmd_inst_opcode_i(core_cmd_inst_opcode_chan);
m_vv_add-> core_cmd_rs1_i(core_cmd_rs1_chan);
m_vv_add-> core_cmd_rs2_i(core_cmd_rs2_chan);

m_vv_add-> core_resp_ready_i(core_resp_ready_chan);
m_vv_add-> core_resp_valid_o(core_resp_valid_chan);
m_vv_add-> core_resp_rd_o(core_resp_rd_chan);
m_vv_add-> core_resp_data_o(core_resp_data_chan);

m_vv_add-> mem_req_ready_i(mem_req_ready_chan);
m_vv_add-> mem_req_valid_o(mem_req_valid_chan);
m_vv_add-> mem_req_addr_o(mem_req_addr_chan);
m_vv_add-> mem_req_tag_o(mem_req_tag_chan);
m_vv_add-> mem_req_cmd_o(mem_req_cmd_chan); //0x000 store 0x001 load
m_vv_add-> mem_req_typ_o(mem_req_typ_chan); //width of response 0x000---8bits 0x001---16bits 0x010---32bits 0x011---64bits
m_vv_add-> mem_req_phys_o(mem_req_phys_chan);
m_vv_add-> mem_req_data_o(mem_req_data_chan); //store data

//for response  can be read from the memory at any time
m_vv_add-> mem_resp_valid_i(mem_resp_valid_chan);
m_vv_add-> mem_resp_addr_i(mem_resp_addr_chan);
m_vv_add-> mem_resp_tag_i(mem_resp_tag_chan);
m_vv_add-> mem_resp_cmd_i(mem_resp_cmd_chan); //0x000 store 0x001 load
m_vv_add-> mem_resp_typ_i(mem_resp_typ_chan); //width of response 0x000---8bits 0x001---16bits 0x010---32bits 0x011---64bits

m_vv_add-> mem_resp_data_i(mem_resp_data_chan); //the data loaded from the em
m_vv_add-> mem_resp_has_data_i(mem_resp_has_data_chan);       //Set if the data field is valid in response
m_vv_add-> mem_resp_data_word_bypass_i(mem_resp_data_word_bypass_chan);   //bypassed to a read???
m_vv_add-> mem_resp_store_data_i(mem_resp_store_data_chan);        //return the data stored
